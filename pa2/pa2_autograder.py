...
#    print "Grading", dirname
    ...
    if not os.path.isfile("first"):
        print "Executable %s missing. Please check the compilation output."%("first")
	os.chdir(prevdir)
	print "Score is ", score
	print ""
	print ""
    ...
    for test in list_tests:        
        ...
        ret = "";
        try:
            command_string = "./first " + train_file + " " + test_dir + test
...
def global_grade(dirname):
	...
	#print "looking now for -", os.path.join(subdir), "-"
        ...
        if subdir == 'first':
            print  "Grading first"
            first_score = first_grade(subdir)
            final_release_scores['first'] = first_score
            print "test score:", final_release_scores['first']
...
#            	print "Deleting the directory obj_temp."
    ...
    total_score = final_release_scores['first'] + final_release_scores['second']
    extra_credit_score = final_release_scores['third']
    print "Your TOTAL SCORE = ", total_score, "/50"
    print "Your EXTRA CREDIT SCORE = ", extra_credit_score, "/12.5"
